...
class ppo_worker():
    def __init__(self, render, quad_worker, cv_cam, ldg_policy, child = False):
    ...
    def wait_until_ready(self, task):        
        if self.quad_worker.update[0] and not self.quad_worker.visual_done:
            self.step(self.quad_worker) 
            self.quad_worker.update = [0, None]
            self.quad_worker.wait_for_task = False 
        return task.cont
